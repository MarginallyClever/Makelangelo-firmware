...
int delta_calcAngleYZ(float x0, float y0, float z0, long &theta) {
  ...
  // z = a + b*y
  float a = (x0 * x0 + y0 * y0 + z0 * z0 + SHOULDER_TO_ELBOW * SHOULDER_TO_ELBOW - ELBOW_TO_WRIST * ELBOW_TO_WRIST -
             y1 * y1) /
            (2.0 * z0);
  float b = (y1 - y0) / z0;
...
void IK(const float *const axies, long *motorStepArray) {
  ...
  if (state == 0) {
    state = delta_calcAngleYZ(x * COS120 + y * SIN120, y * COS120 - x * SIN120, z,
                              motorStepArray[1]);  // rotate coords to +120 deg
  }
  if (state == 0) {
    state = delta_calcAngleYZ(x * COS120 - y * SIN120, y * COS120 + x * SIN120, z,
                              motorStepArray[2]);  // rotate coords to -120 deg
  }
