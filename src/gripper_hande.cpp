...
#  define GRIPPER_REG_POSITION_REQUEST \
    2  // 0...255.  0 is fully open, 255 is fully closed. 0.2mm per bit, 50mm range.  set this before setting rGTO.
...
// function messages
// Function code 04 (FC04) is used for requesting the status of the gripper analog input register. Examples of such data
// are gripper status, object status, finger position, etc. read values: slave id, 4, address of first read register,
// number of read registers, CRC response:    slave id, 4, address of first read register, number of read registers,
// [bytes of data], CRC
...
// set values:  slave id, 23, address of first read register, number of read registers, [bytes of data], address of
// first write register, number of write registers, [bytes of data], CRC response:    slave id, 23, address of first
// read register, number of read registers, [bytes of data], CRC
...
/*
  ...
  #define GRIPPER_BIT_rACT             0  // enable pin?  Reset to clear fault status.
  #define GRIPPER_BIT_rGTO             3  // go to desired position.
  #define GRIPPER_BIT_rATR             4  // automatic release - slow opening of all grippers to maximum.  Emergency
  auto-release. #define GRIPPER_BIT_rARD             5  // auto release direction.  high for open, low for close.
*/
...
void Gripper::readResponse(int bytesExpected) {
  ...
  if (arrived == bytesExpected) {
    ...
    if (inBuf[0] != GRIPPER_SLAVE_ID) { PRINT("Bad slave id"); }
  }
...
uint16_t Gripper::crc1(uint8_t bytes, uint16_t len) {
  uint16_t crc = 0xFFFF;
  for (int i = 0; i < arrived - 2; ++i) { crc = crc16_update(crc, inBuf[i]); }
  return crc;
}
